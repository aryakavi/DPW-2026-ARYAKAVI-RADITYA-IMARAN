...
Buka `http://localhost:8000/index.html`. Jika terminal berada di folder induk, URL harus menyertakan nama folder proyek, misalnya `http://localhost:8000/Jobsheet-6-KegiatanKita/kegiatan/list.html`.
...
1. Buka `kegiatan/list.html` lalu DevTools → Network, refresh.
2. Periksa permintaan `kegiatan.json`: status 200 dan respons array 10 objek.
3. Pada peserta, periksa `peserta.json`: array 4 objek.
4. Pada pendaftaran, periksa `pendaftaran.json`, `peserta.json`, dan `kegiatan.json`: ketiganya status 200.
5. Teks loading terlihat selama simulasi 600ms; UI tetap bisa digunakan.
6. Filter yang diketik selama loading diterapkan ketika respons selesai.

## Menguji kegagalan

- Sementara ubah `fetch("../data/kegiatan.json")` dalam `assets/js/kegiatan.js` ke nama yang tidak ada. Refresh → pesan status 404 dalam tabel, loading berhenti.
